...
public class OpenCVAdvancedTracking : MonoBehaviour
{
    ...
    void initOpenCV()
    {
        ...
        int result = OpenCVInterop.Init(ref camWidth, ref camHeight);
            ...
            if (result == 1)
            {
                Debug.Log("[{1}] Successfully Setup");
            }
            if (result == -1)
            {
            ...
            {
                Debug.LogWarningFormat("[{-2}] Failed to open camera stream.", GetType());
            }
            else if (result == -3)
            {
                Debug.LogWarningFormat("[{-3}] No Bodies Detected.", GetType());
            }
            else if (result == -4)
            {
                Debug.LogWarningFormat("[{-4}] Tracking Error.", GetType());
            }
    ...
    void PatientTracking()
    {
        ...
        unsafe
        {
            ...
            {
               OpenCVInterop.Detect(outTracking, _maxTrackCount, ref detectedTrackingCount);
            }
        }
